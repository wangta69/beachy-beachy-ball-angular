 ...
 * @category Components
 */
export const Physics: any = (props: any) => {
// export const Physics: FC<PhysicsProps> = (props) => {
  ...
  const rigidBodyStates:RigidBodyStateMap = new Map();
  const colliderStates:ColliderStateMap = new Map();
  const rigidBodyEvents:EventMap  = new Map();
  const colliderEvents:EventMap = new Map();
  const eventQueue:EventQueue = new EventQueue(false);
  const beforeStepCallbacks:WorldStepCallbackSet = new Set();
  const afterStepCallbacks: WorldStepCallbackSet = new Set();
